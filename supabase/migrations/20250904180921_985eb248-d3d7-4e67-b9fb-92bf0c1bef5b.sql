...
create policy if not exists "Users can view their own whatsapp instances"
  on public.whatsapp_instances for select
  to authenticated
  using (auth.uid() = user_id);

create policy if not exists "Users can insert their own whatsapp instances"
  on public.whatsapp_instances for insert
  to authenticated
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own whatsapp instances"
  on public.whatsapp_instances for update
  to authenticated
  using (auth.uid() = user_id);

create policy if not exists "Users can delete their own whatsapp instances"
...
create trigger if not exists trg_whatsapp_instances_updated_at
...
create policy if not exists "Users can view their own lives"
  on public.lives for select
  to authenticated
  using (auth.uid() = user_id);

create policy if not exists "Users can insert their own lives"
  on public.lives for insert
  to authenticated
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own lives"
  on public.lives for update
  to authenticated
  using (auth.uid() = user_id);

create policy if not exists "Users can delete their own lives"
...
create trigger if not exists trg_lives_updated_at
...
create policy if not exists "Users can view their own criativos"
  on public.criativos for select
  to authenticated
  using (auth.uid() = user_id);

create policy if not exists "Users can insert their own criativos"
  on public.criativos for insert
  to authenticated
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own criativos"
  on public.criativos for update
  to authenticated
  using (auth.uid() = user_id);

create policy if not exists "Users can delete their own criativos"
...
create policy if not exists "Users can view their own grupos"
  on public.grupos for select
  to authenticated
  using (auth.uid() = user_id);

create policy if not exists "Users can insert their own grupos"
  on public.grupos for insert
  to authenticated
  with check (auth.uid() = user_id);

create policy if not exists "Users can update their own grupos"
  on public.grupos for update
  to authenticated
  using (auth.uid() = user_id);

create policy if not exists "Users can delete their own grupos"
