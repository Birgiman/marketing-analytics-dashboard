...
DROP POLICY IF EXISTS "Users can view campaign links for own lives" ON public.live_campaigns;
...
DROP POLICY IF EXISTS "Users can insert campaign links for own lives" ON public.live_campaigns;
...
DROP POLICY IF EXISTS "Users can update campaign links for own lives" ON public.live_campaigns;
...
DROP POLICY IF EXISTS "Users can delete campaign links for own lives" ON public.live_campaigns;
...
-- Update timestamp trigger (only if function exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'update_updated_at_column') THEN
        DROP TRIGGER IF EXISTS update_live_campaigns_updated_at ON public.live_campaigns;
