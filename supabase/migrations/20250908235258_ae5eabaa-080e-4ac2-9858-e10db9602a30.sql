-- Create role enum if not exists with simpler values
DO $$
BEGIN
...
FOR SELECT
USING (
  auth.uid() = user_id OR public.has_role(auth.uid(), 'admin') OR public.has_role(auth.uid(), 'super')
);
...
SET role = 'super', email = COALESCE(p.email, 'admin@liveshop.com')
