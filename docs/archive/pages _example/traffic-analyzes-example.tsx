...
const TrafficAnalysis = () => {
  ...
  // Estado para controlar o nível de visualização (campanhas, conjuntos, criativos)
  const [viewLevel, setViewLevel] = useState<'campaigns' | 'adsets' | 'creatives'>('adsets');
  ...
  // Calcular totais baseados nos dados filtrados da tabela
  const calculateFilteredTotals = (filteredData: any[]) => {
    ...
    const totalInvestment = filteredData.reduce((sum, day) => sum + day.investment, 0);
    const totalLeads = filteredData.reduce((sum, day) => sum + day.cadastros, 0);
    const totalGroup = filteredData.reduce((sum, day) => sum + day.group, 0);
    const totalGroupExit = filteredData.reduce((sum, day) => sum + day.groupExit, 0);
  ...
  const calculateDailyData = () => {
    ...
    // Filtrar criativos por público selecionado
    const filteredCriativosData = selectedPublico.includes('todos') ? criativosData : criativosData.filter(item => selectedPublico.includes(item.praca));
    ...
    if (gruposData && Array.isArray(gruposData)) {
      console.log('📊 DEBUG GRUPOS - Total de registros:', gruposData.length);
      console.log('📊 DEBUG GRUPOS - Primeiros 5 registros:', gruposData.slice(0, 5));

      // Filtrar por público se selecionado
      const filteredGruposData = selectedPublico.includes('todos') ? gruposData : gruposData.filter(grupo => selectedPublico.includes(grupo.praca));
      ...
      filteredGruposData.forEach((grupo, index) => {
        if (grupo.evento === 'ENTROU' && grupo.data) {
          ...
          if (fullDateKey) {
            // Se a data não existe em dailyData, criar entrada para ela
            if (!dailyData[fullDateKey]) {
              dailyData[fullDateKey] = {
                date: fullDateKey,
                investment: 0,
            ...
            console.log('❌ DEBUG ERRO - Não foi possível converter a data:', grupo.data);
          }
        }
      });
  ...
  const totals = calculateFilteredTotals(filteredTableData);
  
  // Calcular CPL Meta e CPL Líquido baseados nos dados filtrados
  const cplMeta = totals.totalLeads > 0 ? totals.totalInvestment / totals.totalLeads : 0;
  const cplLiquido = totals.totalGroup > 0 ? totals.totalInvestment / totals.totalGroup : 0;

  // Calcular médias baseadas nos totais dos dados filtrados (não média simples)
  const calculateAverages = () => {
    // CPL Meta = Total Investimento / Total Cadastros
    const avgCplMeta = totals.totalLeads > 0 ? totals.totalInvestment / totals.totalLeads : 0;
    
    // CPL Líquido = Total Investimento / Total que Entrou no Grupo
    const avgCplLiquido = totals.totalGroup > 0 ? totals.totalInvestment / totals.totalGroup : 0;
    
    // Taxa de Retenção = (Total que Entrou no Grupo / Total Cadastros) * 100
    const avgRetention = totals.totalLeads > 0 ? (totals.totalGroup / totals.totalLeads) * 100 : 0;
  ...
  // Calcular dados por nível (campanhas, conjuntos, criativos)
  const calculateAnalysisData = () => {
    if (!criativosData || !Array.isArray(criativosData) || criativosData.length === 0) {
      return [];
    }

    // Filtrar por público primeiro
    let filteredCriativosData = selectedPublico.includes('todos') ? criativosData : criativosData.filter(item => selectedPublico.includes(item.praca));
    
    // Depois filtrar dados por data se houver filtro ativo
    if (adSetStartDate && adSetEndDate) {
      filteredCriativosData = filteredCriativosData.filter(item => {
    ...
    if (viewLevel === 'campaigns') {
      ...
      filteredCriativosData.forEach(item => {
        if (!item.campaign_name) return;
        const campaignKey = item.campaign_name;
        if (!campaignData[campaignKey]) {
          campaignData[campaignKey] = {
            ...
            total_leads: 0,
            cpl: 0,
            adsets_count: new Set(),
            creatives_count: new Set()
          };
        }
        const amount = typeof item.amount_spent === 'string' ? parseFloat(item.amount_spent) || 0 : item.amount_spent !== null && item.amount_spent !== undefined ? item.amount_spent : 0;
        const leads = item.leads !== null && item.leads !== undefined ? typeof item.leads === 'string' ? parseInt(item.leads) || 0 : item.leads : 0;
        campaignData[campaignKey].total_spent += amount;
        campaignData[campaignKey].total_leads += leads;
        if (item.ad_set_name) campaignData[campaignKey].adsets_count.add(item.ad_set_name);
        if (item.ad_name) campaignData[campaignKey].creatives_count.add(item.ad_name);
      });

      // Calcular CPL e converter Sets para contadores, depois filtrar por texto
      let campaigns = Object.values(campaignData).map((campaign: any) => ({
        ...campaign,
        adsets_count: campaign.adsets_count.size,
        creatives_count: campaign.creatives_count.size,
        cpl: campaign.total_leads > 0 ? campaign.total_spent / campaign.total_leads : 999999
      })).filter((campaign: any) => campaign.total_leads > 0);

      // Filtrar por texto se houver filtro
      if (campaignFilter) {
      ...
      return campaigns.sort((a: any, b: any) => a.cpl - b.cpl);
    } 
      ...
      filteredCriativosData.forEach(item => {
        ...
        if (item.ad_name) adSetData[adSetKey].creatives_count.add(item.ad_name);
      });

      // Calcular CPL e converter Set para contador, depois filtrar por texto
      let adSets = Object.values(adSetData).map((adSet: any) => ({
        ...adSet,
        creatives_count: adSet.creatives_count.size,
        cpl: adSet.total_leads > 0 ? adSet.total_spent / adSet.total_leads : 999999
      })).filter((adSet: any) => adSet.total_leads > 0);

      // Filtrar por texto se houver filtro
      if (adSetFilter) {
        adSets = adSets.filter((adSet: any) => 
          adSet.name.toLowerCase().includes(adSetFilter.toLowerCase()) ||
          adSet.campaign_name.toLowerCase().includes(adSetFilter.toLowerCase())
        );
      }

      return adSets.sort((a: any, b: any) => a.cpl - b.cpl);
    } 
      ...
      filteredCriativosData.forEach(item => {
        if (!item.ad_name) return;
        const creativeKey = item.ad_name;
        if (!creativeData[creativeKey]) {
        ...
        const amount = typeof item.amount_spent === 'string' ? parseFloat(item.amount_spent) || 0 : item.amount_spent !== null && item.amount_spent !== undefined ? item.amount_spent : 0;
        const leads = item.leads !== null && item.leads !== undefined ? typeof item.leads === 'string' ? parseInt(item.leads) || 0 : item.leads : 0;
        creativeData[creativeKey].total_spent += amount;
        creativeData[creativeKey].total_leads += leads;
      });

      // Calcular CPL e filtrar por texto
      let creatives = Object.values(creativeData).map((creative: any) => ({
        ...creative,
        cpl: creative.total_leads > 0 ? creative.total_spent / creative.total_leads : 999999
      })).filter((creative: any) => creative.total_leads > 0);

      // Filtrar por texto se houver filtro
      if (creativeFilter) {
        creatives = creatives.filter((creative: any) => 
          creative.name.toLowerCase().includes(creativeFilter.toLowerCase()) ||
          creative.campaign_name.toLowerCase().includes(creativeFilter.toLowerCase()) ||
          creative.ad_set_name.toLowerCase().includes(creativeFilter.toLowerCase())
        );
      }

      return creatives.sort((a: any, b: any) => a.cpl - b.cpl);
    }
  };
  const analysisData = calculateAnalysisData();

  // Filtrar dados baseado no nível atual
  const filteredAdSets = analysisData;
  ...
  // Calcular totais para os cabeçalhos da tabela 
  const calculateAdSetTotals = () => {
    ...
    const totalLeads = sortedAdSets.reduce((sum, item) => sum + item.total_leads, 0);
    const totalSpent = sortedAdSets.reduce((sum, item) => sum + item.total_spent, 0);
    const averageCPL = sortedAdSets.reduce((sum, item) => sum + item.cpl, 0) / sortedAdSets.length;
    return {
      totalLeads,
      totalSpent,
      averageCPL
  ...
  return <div className="container mx-auto p-6 space-y-8">
              ...
              <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div className="relative" ref={dropdownRef}>
                    <button 
                      className="px-3 py-2 text-sm border border-input rounded-md bg-background min-w-[200px] flex items-center justify-between hover:bg-accent"
                      onClick={() => setShowPublicoDropdown(!showPublicoDropdown)}
                    >
                      <span>
                        {selectedPublico.includes('todos') ? 'Todos os Públicos' : 
                         selectedPublico.length === 1 ? 
                         (selectedPublico[0] === 'ES' ? 'Espírito Santo' :
                          selectedPublico[0] === 'MA' ? 'Maceió' :
                          selectedPublico[0] === 'BR' ? 'Brasília' :
                          selectedPublico[0] === 'NA' ? 'Nacional' :
                          selectedPublico[0] === 'NAB' ? 'Nacional Teste' : selectedPublico[0]) :
                         `${selectedPublico.length} públicos selecionados`}
                      </span>
                          ...
                          <div
                            ...
                            onClick={() => {
                              if (option.value === 'todos') {
                                ...
                                const newSelection = selectedPublico.includes(option.value)
                                  ? selectedPublico.filter(p => p !== option.value && p !== 'todos')
                                  : [...selectedPublico.filter(p => p !== 'todos'), option.value];
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-center">
                     ...
                     <TableHead className="text-center">
                       <Button variant="ghost" onClick={() => handleSort('cplMeta')} className="h-auto p-0 font-medium flex flex-col items-center gap-1">
                         <div className="text-center">
                           <div>CPL Meta</div>
                           <div className="text-xs text-muted-foreground font-normal">Média: R$ {averages.avgCplMeta.toFixed(2).replace('.', ',')}</div>
                         </div>
                         {getSortIcon('cplMeta')}
                       </Button>
                     </TableHead>
                     <TableHead className="text-center">
                       <Button variant="ghost" onClick={() => handleSort('cplLiquido')} className="h-auto p-0 font-medium flex flex-col items-center gap-1">
                         <div className="text-center">
                           <div>CPL Líquido</div>
                           <div className="text-xs text-muted-foreground font-normal">Média: R$ {averages.avgCplLiquido.toFixed(2).replace('.', ',')}</div>
                         </div>
                         {getSortIcon('cplLiquido')}
                       </Button>
                     </TableHead>
                     <TableHead className="text-center">
                       <Button variant="ghost" onClick={() => handleSort('retention')} className="h-auto p-0 font-medium flex flex-col items-center gap-1">
                         <div className="text-center">
                           <div>Taxa Retenção</div>
                           <div className="text-xs text-muted-foreground font-normal">Média: {averages.avgRetention.toFixed(1)}%</div>
                         </div>
              ...
              <div className="space-y-3">
                ...
                {cplLiquido < 2.00 ? <div className="p-4 border-l-4 border-green-500 bg-green-50 dark:bg-green-950">
                    ...
                    <p className="text-sm text-green-600 dark:text-green-400">
                      CPL Líquido (R$ {cplLiquido.toFixed(2)}) está {((1 - cplLiquido / 2.00) * 100).toFixed(0)}% abaixo da meta de R$ 2,00. Escalar gradualmente os conjuntos de anúncios com melhor performance.
                    </p>
                    ...
                    <p className="text-sm text-red-600 dark:text-red-400">
                      CPL Líquido (R$ {cplLiquido.toFixed(2)}) está {((cplLiquido / 2.00 - 1) * 100).toFixed(0)}% acima da meta de R$ 2,00. Pausar conjuntos com pior performance e otimizar criativos.
                    </p>
                  </div>}
                ...
                {/* Análise dos Melhores */}
                {analysisData.length > 0 && <div className="p-4 border-l-4 border-blue-500 bg-blue-50 dark:bg-blue-950">
                    <h5 className="font-semibold text-blue-700 dark:text-blue-300">
                      🎯 Melhor {viewLevel === 'campaigns' ? 'Campanha' : viewLevel === 'adsets' ? 'Conjunto de Anúncios' : 'Criativo'}
                    </h5>
                    <p className="text-sm text-blue-600 dark:text-blue-400">
                      "{analysisData[0].name}" com CPL de R$ {analysisData[0].cpl.toFixed(2)}. 
                      {analysisData[0].cpl < 0.70 ? ' Aumentar orçamento em 30-50% para escalar.' : ' Analisar elementos de sucesso para replicar.'}
                    </p>
                  </div>}
              </div>
            </CardContent>
        ...
        {/* Analysis Table with Level Selection */}
        <Card>
          <CardHeader>
            <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
              <div>
                <CardTitle>🏆 Análise Profunda de {viewLevel === 'campaigns' ? 'Campanhas' : viewLevel === 'adsets' ? 'Conjuntos de Anúncios' : 'Criativos'}</CardTitle>
              </div>
              <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4">
                {/* Level Selection Buttons */}
                <div className="flex items-center space-x-2">
                  <Button 
                    variant={viewLevel === 'campaigns' ? 'default' : 'outline'} 
                    size="sm"
                  ...
                  <Button 
                    variant={viewLevel === 'adsets' ? 'default' : 'outline'} 
                    size="sm"
                  ...
                  <Button 
                    variant={viewLevel === 'creatives' ? 'default' : 'outline'} 
                    size="sm"
                    onClick={() => setViewLevel('creatives')}
                  >
                    Criativos
                  </Button>
                </div>

                {/* Filtro de texto específico por nível */}
                <div className="flex items-center space-x-2">
                  <Input 
                    placeholder={`Filtrar ${viewLevel === 'campaigns' ? 'campanhas' : viewLevel === 'adsets' ? 'conjuntos' : 'criativos'}...`}
                    value={viewLevel === 'campaigns' ? campaignFilter : viewLevel === 'adsets' ? adSetFilter : creativeFilter}
                    onChange={(e) => {
                      if (viewLevel === 'campaigns') setCampaignFilter(e.target.value);
                      else if (viewLevel === 'adsets') setAdSetFilter(e.target.value);
                      else setCreativeFilter(e.target.value);
                    }}
                    className="w-64"
                  />
                </div>
          ...
          <CardContent>
            <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>
                      <Button variant="ghost" onClick={() => handleAdSetSort('name')} className="h-auto p-0 font-medium flex items-center gap-1">
                        {viewLevel === 'campaigns' ? 'Campanha' : viewLevel === 'adsets' ? 'Conjunto de Anúncios' : 'Criativo'}
                        {getAdSetSortIcon('name')}
                      </Button>
                    </TableHead>
                    {viewLevel === 'campaigns' && (
                      <>
                        <TableHead className="text-center">
                          <Button variant="ghost" onClick={() => handleAdSetSort('adsets_count')} className="h-auto p-0 font-medium flex items-center gap-1">
                            Conjuntos
                            {getAdSetSortIcon('adsets_count')}
                          </Button>
                        </TableHead>
                        <TableHead className="text-center">
                          <Button variant="ghost" onClick={() => handleAdSetSort('creatives_count')} className="h-auto p-0 font-medium flex items-center gap-1">
                            Criativos
                            {getAdSetSortIcon('creatives_count')}
                          </Button>
                        </TableHead>
                      </>
                    )}
                ...
                <TableBody>
                  {sortedAdSets.map((item, index) => (
                    <TableRow key={index}>
                      <TableCell>
                        <div>
                          <div className="font-semibold">{item.name}</div>
                          {viewLevel !== 'campaigns' && (
                            <div className="text-xs text-muted-foreground">{item.campaign_name}</div>
                          )}
                          {viewLevel === 'creatives' && (
                            <div className="text-xs text-muted-foreground">{item.ad_set_name}</div>
                          )}
                        </div>
                      </TableCell>
                      {viewLevel === 'campaigns' && (
                        <>
                          <TableCell className="text-center font-medium">{item.adsets_count}</TableCell>
                          <TableCell className="text-center font-medium">{item.creatives_count}</TableCell>
                        </>
                      )}
                      {viewLevel === 'adsets' && (
                        <TableCell className="text-center font-medium">{item.creatives_count}</TableCell>
                      )}
                      <TableCell className="text-center font-medium">{item.total_leads}</TableCell>
                      <TableCell className="text-center font-medium">R$ {item.total_spent.toFixed(2).replace('.', ',')}</TableCell>
                      <TableCell className="text-center font-medium">
                        R$ {item.cpl.toFixed(2).replace('.', ',')}
                      </TableCell>
                      {(viewLevel === 'creatives') && (
                        <TableCell className="text-center">
                          {item.creative_link ? (
                            <Button variant="outline" size="sm" asChild>
                              <a href={item.creative_link} target="_blank" rel="noopener noreferrer">
                  ...
                  {sortedAdSets.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={viewLevel === 'campaigns' ? 5 : viewLevel === 'adsets' ? 6 : 6} className="text-center text-muted-foreground">
                        {isLoading ? 'Carregando dados...' : `Nenhum${viewLevel === 'campaigns' ? 'a campanha' : viewLevel === 'adsets' ? ' conjunto de anúncios' : ' criativo'} encontrado${viewLevel === 'campaigns' ? '' : viewLevel === 'adsets' ? '' : ''}`}
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
